...
from aos_agent.bio_mas import bio_mas_preflight, create_tiny_bio_demo, run_tiny_bio_mas_demo
from aos_agent.data.catalog import build_dataset_catalog
from aos_agent.data.labels import build_seaad_label_maps
from aos_agent.data.prepare_sources import prepare_generation_sources
from aos_agent.data.reference import build_reference, build_seaad_test_h5ad
from aos_agent.eval.datasets import prepare_all_eval_datasets
from aos_agent.eval.label_transfer import evaluate_raw_label_transfer
from aos_agent.eval.run import run_evaluation
from aos_agent.eval.uce_ima import run_uce_ima_label_transfer
from aos_agent.io import write_json
from aos_agent.scdesign3.generate import preflight_scdesign3, run_generation_configs, write_generation_configs
from aos_agent.stage2.selector import profile_query, run_cross_species_plan, select_models
from aos_agent.stage3.adapter_executor import adapt_and_execute, inspect_model_contracts
from aos_agent.stage4.consensus import run_consensus
...
def main(argv: list[str] | None = None) -> None:
    ...
    p.add_argument("--include-default-excluded", action="store_true", help="Allow default-excluded lightweight baseline methods; mainly for tiny smoke tests.")
    p.add_argument("--seed", type=int, default=3028)
    ...
    p = sub.add_parser("bio-mas-preflight", help="Inspect AutOmicScience bio MAS dependencies, R, real data, checkpoints, and tiny-demo assets.")
    p.add_argument("--output-dir", default="")
    p.add_argument("--rscript-path", default="")
    p.add_argument("--no-foundation", action="store_true")
    p.add_argument("--no-notebook", action="store_true")

    p = sub.add_parser("create-tiny-bio-demo", help="Create clearly marked synthetic tiny data for local MAS smoke tests.")
    p.add_argument("--output-dir", default="")
    p.add_argument("--cells-per-label", type=int, default=8)
    p.add_argument("--seed", type=int, default=3028)

    p = sub.add_parser("run-tiny-bio-mas-demo", help="Run profile -> select -> execute on synthetic tiny demo data.")
    p.add_argument("--output-dir", default="")
    p.add_argument("--cells-per-label", type=int, default=8)
    p.add_argument("--seed", type=int, default=3028)
    ...
    elif args.cmd == "bio-mas-preflight":
        result = bio_mas_preflight(
            output_dir=args.output_dir or None,
            rscript_path=args.rscript_path or "",
            include_foundation=not args.no_foundation,
            include_notebook=not args.no_notebook,
        )
    elif args.cmd == "create-tiny-bio-demo":
        result = create_tiny_bio_demo(
            output_dir=args.output_dir or None,
            cells_per_label=args.cells_per_label,
            seed=args.seed,
        )
    elif args.cmd == "run-tiny-bio-mas-demo":
        result = run_tiny_bio_mas_demo(
            output_dir=args.output_dir or None,
            cells_per_label=args.cells_per_label,
            seed=args.seed,
            top_k=args.top_k,
