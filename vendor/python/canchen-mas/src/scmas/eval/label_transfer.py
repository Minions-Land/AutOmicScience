...
from scmas.io import ensure_dir, normalize_gene_name, read_json, read_standard_bundle, stratified_indices, write_json
...
class UCE33LRawEmbedding(UCE4LRawEmbedding):
    def __init__(
        ...
        *,
        ...
        model_dir = Path(model_dir)
        full_state = torch.load(model_dir / "33l_8ep_1024t_1280.torch", map_location="cpu")
        backbone_state = {k: v for k, v in full_state.items() if not k.startswith("pe_embedding")}
        missing, unexpected = self.backbone.load_state_dict(backbone_state, strict=False)
        del full_state
